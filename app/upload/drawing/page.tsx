...
type DrawingType = 'CUSTOMER_DRAWING' | 'INTERNAL_DRAWING';
      ...
      <div className="space-y-8 max-w-2xl">
        <header className="flex flex-col gap-2">
          ...
          <p className="text-gray-600">
            Upload a customer or internal drawing PDF. The system will derive the part number and create or match an SKU
            stub automatically.
          </p>
        </header>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-4">
          ...
          >
            {uploading ? (
              <span className="text-sm">Processing drawing...</span>
            ) : (
              <span className="text-sm font-medium">Click to select drawing PDF</span>
            )}
          </button>
        ...
        {needsPN && (
          <section className="bg-yellow-50 border border-yellow-200 rounded-2xl p-6 space-y-4">
            <p className="text-sm text-yellow-800 font-medium">
              Could not auto-detect part number from the drawing. Enter it manually to continue.
            </p>
            ...
            <div className="rounded-xl border border-dashed border-gray-200 px-4 py-3 text-sm text-gray-600 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-semibold rounded-full px-2 py-0.5 ${
                  ...
                  {result.pipeline_status === 'READY' ? 'READY' : 'WAITING FOR BOM'}
                </span>
              </div>
              {result.pipeline_status === 'READY' ? (
                <p>
                  HWI job refreshed with this drawing. Review from the SKU page or Work Instructions view.
                </p>
              ) : (
                <p>
                  Upload the matching BOM to auto-build harness instructions for this SKU.
                </p>
              )}
            </div>

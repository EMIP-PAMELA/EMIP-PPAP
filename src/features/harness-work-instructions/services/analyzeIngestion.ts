...
export async function analyzeFileIngestion(params: AnalyzeIngestionParams): Promise<IngestionAnalysisResult> {
  console.log('🧪🧪🧪 BUILD ID: R8-INGESTION-A 🧪🧪🧪');
  ...
  const parserInventory = {
    coreBomParser: false,
    wireTableParser: false,
    projectionParser: false,
    adaptiveParser: false,
  };

  const ENABLE_BOM_WIRE_TABLE_PARSER = false;
  const ENABLE_WIRE_RECOVERY_FALLBACK = false;
  const ENABLE_VISION_DIAGRAM_MERGE = false;
  const ENABLE_VISION_CONNECTIVITY_BRIDGE = false;
